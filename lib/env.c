                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    for (i = 0; i < bin_size; i += BY2PG) {
        ...
        p->pp_ref++;
        if (i == 0) {
            bcopy(bin, (char *)page2kva(p) + offset, ((BY2PG - offset) < bin_size) ? (BY2PG - offset) : bin_size);
            /*if(BY2PG-offset > bin_size){
                bzero((char *)page2kva(p) + offset + bin_size, BY2PG - offset - bin_size);
            }*/
    ...
    /*
        ...
        if(page_insert(env->env_pgdir,p,ROUNDDOWN(va,BY2PG)+i*BY2PG,0)!=0)
            return -1;
        char *p_t = (char *)page2kva(p);
        int j;
        if(i==0){
            if(bin_size+offset<BY2PG){
                for(j=offset;j<bin_size+offset;j++){
                    p_t[j] = *bin_t;
                    bin_t++;
                }

            }else{
                for(j=offset;j<BY2PG-offset;j++){
                    p_t[j] = *bin_t;
                    bin_t++;
                }

            }
        } else if(i+BY2PG-offset>=bin_size){
            for(j=0;j<bin_size-i+offset;j++){
                p_t[j] = *bin_t;
                bin_t++;
            }
    ...
    * i has the value of `bin_size` now. */
    while (i < sgsize) {
        ...
        p->pp_ref++;
        r = page_insert(env->env_pgdir, p, va + i, PTE_V | PTE_R);
        //r = page_insert(env -> env_pgdir, p, ROUNDDOWN(va,BY2PG)+i*BY2PG, PTE_V | PTE_R);
        if (r != 0) {
            return -E_NO_MEM;
        }
...
void env_run(struct Env *e)
{
    ...
    *  context switch.You can imitate env_destroy() 's behaviors.*/
    struct Trapframe *temp = (struct Trapframe *)(TIMESTACK - sizeof(struct Trapframe));
    if (curenv != NULL) {
