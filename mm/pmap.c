...
void page_init(void)
{
     ...
     * filed to 1) */
    int i;
    u_long n_used_page = PADDR(freemem) / BY2PG;
    for (i = 0; i < n_used_page; i++) {
        (pages + i)->pp_ref = 1;
    }

    /* Step 4: Mark the other memory as free. */
    for (i = n_used_page; i < npage; i++) {
        (pages + i)->pp_ref = 0;
...
void pageout(int va, int context)
{
    ...
    p->pp_ref++; // pp_ref记录指向该物理页的虚页数量，在page_insert函数中会加1，不需要在此处加1
